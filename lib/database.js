...
class Database {
  ...
  // Retrieving a record
  async get(id, recordType = 'untyped') {
  ...
  async add(record) {
    ...
    record._id = record._id || nanoid()
    record._type = record._type || 'untyped'
    record._created_at = record._created_at || Date.now()
    record._files = record._files || []
